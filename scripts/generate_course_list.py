...
def process_sheet_and_generate_csv(sheet_service):
    ...
    with open(CSV_FILE, "w", newline="", encoding="utf-8") as f:
        ...
        for i in range(max(len(included), len(excluded))):
            writer.writerow([included[i] if i < len(included) else "", excluded[i] if i < len(excluded) else ""])
...
def update_statuses(sheet_service, updates):
    ...
    for row_idx, status, note in updates:
        data.append({"range": f"Sheet1!D{row_idx}", "values": [[status]]})
        data.append({"range": f"Sheet1!F{row_idx}", "values": [[note]]})
